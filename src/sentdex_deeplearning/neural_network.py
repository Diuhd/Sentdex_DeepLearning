...
class Loss_Binary_Cross_Entropy(Loss):
    def forward(self, y_pred, y_true):
        ...
        self.y_pred = y_pred_clipped
        self.y_true = y_true
        sample_outputs = -(y_true * np.log(y_pred_clipped) + (1 - y_true) * np.log(1 - y_pred_clipped))
        sample_outputs = np.mean(sample_outputs, axis=-1)
    ...
    def backward(self, y_pred=None, y_true=None):
        ...
        self.dinputs = -(y_true / y_pred - (1 - y_true) / (1 - y_pred)) / n_outputs # pyright: ignore[reportOperatorIssue]
